...
- **Pitch-corrected speed** — `AVPlayerEngine.open()` sets `item.audioTimePitchAlgorithm = .spectral` so 1.5× / 2× playback doesn't chipmunk; libvlc path passes `:audio-time-stretch` as a media option (default is already on in libvlc 3.x, set explicitly to match). Without this, voices at 2× are unwatchable for lectures/podcasts. `.spectral` is the most expensive algorithm but negligible vs video decode
- **Frame-step backward in libvlc** — libvlc 3.x exposes `libvlc_media_player_next_frame` but has NO `previous_frame` and our compat header doesn't expose `get_fps`, so `VLCPlayerEngine.stepFrame(forward: false)` approximates with a 40ms precise backward seek (≈1/25s — lands within ±1 frame for typical 24/30fps content). AVPlayer path is exact via `step(byCount: -1)`. Don't try to invoke libvlc's previous_frame; it doesn't exist
- **Sleep timer** — `Utilities/SleepTimer.swift` is a singleton with three modes: `.off`, `.duration(minutes:)`, `.endOfFile`. Duration mode is a `Timer.scheduledTimer`; EOF mode is a flag checked at the top of both `playerEngineDidFinishPlaying` and `vlcEngineDidFinishPlaying` BEFORE the existing mediaEndAction switch, so the timer wins over auto-next-track. `SleepTimerMenuDelegate` rebuilds the menu every open so the live "12:34 remaining" countdown updates without timers. Fire handler in `AppDelegate.setSleepTimer` calls `pauseForSleepTimer()` which pauses both engines and OSDs "Sleep Timer — paused" for 5s
- **Trackpad gestures** — `PlayerViewController.scrollWheel(with:)` branches on horizontal-dominant precise deltas (`event.hasPreciseScrollingDeltas` + `|dx| > |dy| * 1.5`) — two-finger trackpad swipe accumulates into 30pt buckets and seeks ±5s per bucket. Mouse wheel sends dx=0 so it skips this branch and falls through to the existing volume/seek logic. Pinch handler rewritten to live-resize the window anchored on its center (`gesture.magnification = 0` after each apply to keep deltas relative), gated on `Defaults.pinchGestureAction != 2`. Both gestures fire during the gesture, not on `.ended` — that's important for the resize "follows your fingers" feel
- **Thumbnail filmstrip on seek hover** — `SeekSliderView` shows a 5-thumb strip (offsets `[-20, -10, 0, +10, +20]` seconds, center scaled 1.25× with blue border) instead of the prior single hover-thumb. Lazy gen via existing `AVAssetImageGenerator`; existing `NSCache` (2s buckets, 24 MB ceiling) is reused. On `currentAsset` change, the panel is torn down (`thumbnailWindow = nil` + clear arrays) so the next hover rebuilds against the new asset — don't try to reuse views, the image refs and slot count tie to the asset. The filmstrip panel is added as a child window of the parent window and clamped to the screen's `visibleFrame` so it doesn't run off-screen at the bar's edges. Time tooltip's `thumbnailOffset` adapts to the strip's real height (`stripThumbHeight * stripCenterScale + 2*stripPadding + 8`)
- **WelcomeView always-dark appearance** — sets `self.appearance = NSAppearance(named: .darkAqua)` in `setupViews()` so child NSButton/NSTextField inherit dark-mode control colors regardless of system theme. Without this, Light Mode renders near-black text on the dark radial gradient and the Open File… / Open URL… / recent-chip labels are unreadable. Don't try to fix it via `attributedTitle` — the system bezel rendering ignores foreground color on `.rounded` / `.inline` bezel styles. Forcing the subtree's NSAppearance is the cleanest approach
...
- **Test target** — wired in pbxproj (T00002, build phases S00002/FB0002/R00002, configs BC0030/BC0031). Shared scheme at `xcshareddata/xcschemes/Awesome Player.xcscheme` so `xcodebuild test` finds it. Three known issues:
  1. **Internal Facebook Xcode builds (`*_fb.app`) on macOS 26 break `xcodebuild test`** with `LaunchServices error -54` ("sandbox profile of this process is missing (allow lsopen)"). This is FB-internal Xcode's sandbox wrapping the `xcodebuild` CLI process, NOT a project bug. Confirmed working environments: Xcode UI Cmd+U on the same machine; stock App Store Xcode 26; Xcode 16 LTS; CI's macos-15 runner. No workaround from the project side — change Xcode or run via UI.
  ...
  3. `ENABLE_DEBUG_DYLIB = NO` is set on the **app** target (both Debug + Release). Xcode 26 auto-enables a debug-launcher dylib when a unit-test target depends on the app, but the resulting `.debug.dylib` lives in `Contents/MacOS/` while the test bundle's `@rpath` doesn't find it (Library not loaded). We don't use iterative test hot-reload, so disabling cuts out the dual-binary path entirely — also makes direct `xcrun xctest "$TEST"` invocations debuggable.
...
- **yt-dlp is stripped — re-run `Scripts/strip-ytdlp.sh` after every yt-dlp upgrade.** The upstream PyInstaller distribution ships every Mach-O as a universal binary (x86_64 + arm64) and bundles `curl_cffi` + `libcurl-impersonate` (~28 MB of browser-impersonation HTTP client we don't need for basic YouTube format listing — yt-dlp falls back to urllib). The script thins everything to arm64 and removes the curl_cffi stack, cutting Vendor/yt-dlp from ~145 MB to ~69 MB with zero behavioral change (same format count and resolution list on the YouTube test video `jNQXAC9IVRw`). If you replace `Vendor/yt-dlp/` with a fresh PyInstaller download from upstream, the strip undoes — run the script again. The script is idempotent so accidental re-runs are fine
